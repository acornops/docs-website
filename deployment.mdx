...
Starter automation is part of control-plane workspace provisioning in every environment. It is independent of optional local target fixtures such as `SEED_DEVELOPMENT_DATA`: each workspace receives starter v1 once, and required startup backfill completes before the control plane reports ready.
...
Before installing the Workflow V2 stack, export both secret-free inventories:

```bash
scripts/agent-capability-cutover-preflight.sh env/vm/.env.prod ./agent-capability-preflight
```

Retain and review `control-plane.json` and `llm-gateway.json`. If the control-plane
report sets `resetRequired: true`, the migration Job aborts before mutation.
Back up, drop, and recreate the external control-plane database explicitly, then
retry with the complete pinned contract-v2 stack matrix. Workflow V1 data is not
preserved or deleted automatically. This is not a rolling upgrade, and mixed
...
- advertises bounded Linux/systemd tools only after authenticated readiness,
- remains read-only unless the separately installed helper, exact policy, run mode, and approval all permit `restart_service`.
