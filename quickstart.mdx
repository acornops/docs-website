...
The management console host also proxies `/api` to the control plane for browser session flows.
...
- TLS certificates and ingress for `api.acornops.dev` and `console.acornops.dev`.
...
Open `https://console.acornops.dev/` and sign in through your configured OIDC provider or username/password auth. If no workspace exists for your account, the management console should prompt you to create one.
