...
flowchart LR
  ...
  LLMGateway --> Postgres
  LLMGateway --> Redis
  WorkloadAgent["AgentK"] -->|Outbound WebSocket| ControlPlane
  VMAgent["AgentV"] -->|Outbound WebSocket| ControlPlane
...
| AgentV | Linux/systemd host snapshots, logs, and read-only builtin VM tool execution | Outbound only |
...
| Live VM host discovery and read-only builtin tool behavior | AgentV |
