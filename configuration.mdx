...
| --- | --- | --- |
| Platform public URL | `https://api.acornops.dev` | Primary API route examples and agent install commands |
...
| Agent WebSocket URL | `wss://api.acornops.dev/api/v1/agent/connect` | Target agent connections |
...
| `EXTERNAL_INTEGRATION_SERVICE_TOKEN` | Registered external integration client access to account-link and resolve endpoints |
...
`EXTERNAL_INTEGRATION_SERVICE_TOKEN` does not authorize general control-plane API calls. It is accepted only by the external integration account-link endpoints used by a registered external integration client.
...
| `ai.allowedReasoningEfforts` | `LLM_ALLOWED_REASONING_EFFORTS` | `default,low,medium,high` |
...
| `AGENT_WRITE_CONFIRMATION_TIMEOUT_SECONDS` | `300` | Bounds how long a paused run waits for a decision. |
...
| `agent.runtime.writeConfirmationTimeoutSeconds` | `300` | Helm value rendered into `AGENT_WRITE_CONFIRMATION_TIMEOUT_SECONDS`. |
