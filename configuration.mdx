...
troubleshooting runs; keep `read_workspace_data` and `create_sessions` because
run creation depends on them. Approval decisions still require an authenticated
AcornOps browser session.
...
required for webhook delivery. Consumers should handle duplicate events and
verify signatures before processing payloads.

| Kubernetes value | Compose variable | Default |
| --- | --- | --- |
| `components.controlPlane.webhookDelivery.enabled` | `WEBHOOK_WORKER_ENABLED` | `true` |
| `components.controlPlane.webhookDelivery.batchSize` | `WEBHOOK_WORKER_BATCH_SIZE` | `50` |
| `components.controlPlane.webhookDelivery.concurrency` | `WEBHOOK_WORKER_CONCURRENCY` | `20` |
| `components.controlPlane.webhookDelivery.perOriginConcurrency` | `WEBHOOK_WORKER_PER_ORIGIN_CONCURRENCY` | `4` |
| `components.controlPlane.webhookDelivery.maxAttempts` | `WEBHOOK_MAX_ATTEMPTS` | `10` |
| `components.controlPlane.webhookDelivery.maxRetryAgeSeconds` | `WEBHOOK_MAX_RETRY_AGE_SECONDS` | `86400` |
| `components.controlPlane.webhookDelivery.maxPayloadBytes` | `WEBHOOK_MAX_PAYLOAD_BYTES` | `65536` |
| `components.controlPlane.webhookDelivery.maxSubscriptionsPerWorkspace` | `WEBHOOK_MAX_SUBSCRIPTIONS_PER_WORKSPACE` | `100` |

Disable the worker to pause claims during maintenance; event enqueue remains
active. Keep insecure HTTP webhook delivery enabled only in explicit local
development.
