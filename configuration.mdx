...
| `MATTERMOST_CHAT_SERVICE_TOKEN` | Registered Mattermost bot access to account-link and resolve endpoints |
...
`MATTERMOST_CHAT_SERVICE_TOKEN` does not authorize general control-plane API calls. It is accepted only by the Mattermost account-link endpoints used by a registered bot.
