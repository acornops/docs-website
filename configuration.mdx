...
There is intentionally no generic `agent` section and no empty AgentV Helm section. Each name identifies one responsibility and one target scope.
...
| `ASSISTANT_WRITE_CONFIRMATION_TIMEOUT_SECONDS` | `assistantRuntime.writeConfirmationTimeoutSeconds` | `900` | Expires Agent, Workflow, and target write approvals after 15 minutes. |
