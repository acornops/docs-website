...
| Store | Contains |
| --- | --- |
| Control-plane Postgres | Workspaces, users, members, targets, Kubernetes clusters, VMs, sessions, runs, invitations, webhooks |
...
Redis is used for coordination, rate limits, control-plane agent ownership, cross-pod command routing, run event fanout, and renewed scheduler leases. Size it for expected run concurrency and event volume.
