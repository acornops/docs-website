...
- Builtin VM tools are provided by the VM agent and synchronized through the control plane.
...
| --- | --- | --- |
| Builtin Kubernetes tools | k8s agent | The connected agent advertises tools and AcornOps shows them in the target tool catalog. |
| Builtin VM tools | VM agent | The agent advertises read-only Linux/systemd tools over `tools/list`; the control plane synchronizes them into the gateway for the VM target. |
...
The default timeout is 300 seconds. Rejection and timeout do not call the k8s agent. Approval executes the queued write once. VM runs are read-only and do not request write approvals.
