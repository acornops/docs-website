...
description: "Detailed platform settings for hosts, auth, model access, runtime policy, egress, and delivery"
---
...
Each remote MCP installation belongs to one workspace Agent or one Kubernetes cluster or VM target. Workflows reuse their selected Agent's installation; they do not own a separate MCP installation or credential. In production, the gateway should require HTTPS and block private, local, and reserved network targets unless you intentionally allow specific hosts.
...
Workspace administrators connect MCP endpoints by URL or enable starters seeded from platform **Workspace Defaults**. Both use the same workspace-managed or individual credential flow. Defaults carry endpoint metadata, not credentials, and later platform-default edits do not update existing workspace snapshots or installations.

The Management Console does not support registry browsing, source management, import, or reimport. Its **Add from registry** action is a disabled **Coming soon** placeholder. The gateway's `components.llmGateway.catalog` configuration and backend registry endpoints remain for future work; configuring them does not enable registry installation in the console. See [MCP registries](/use/mcp-registries) for the supported server sources and [MCP and tools](/use/tools-and-mcp#credential-connections) for credential ownership and OAuth setup.
