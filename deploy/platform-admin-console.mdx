...
<Warning>
  The platform admin console intentionally cannot access tenant logs, workspace audit events, target identities, Agents, sessions, runs, prompts, commands, workspace credentials, or workload controls. Workspace defaults contain only administrator-approved definitions and endpoint metadata; they do not expose or distribute credentials. The console does not support impersonation.
</Warning>
...
Workspace Defaults lets a full platform administrator maintain the MCP server and skill definitions offered when eligible new workspaces are initialized. For MCP servers, the administrator supplies a display name, an HTTPS endpoint, and the supported destinations. The default contains no credential: workspace administrators must explicitly enable the starter to create an installation and configure authentication through the ordinary MCP setup.

Workspace initialization stores a snapshot of the defaults. Later changes to the platform list apply to future workspace initialization, not to existing workspace snapshots or installations. Enabling a starter creates a normal workspace-owned installation that workspace administrators can manage through the same controls as a directly registered server. It is not kept synchronized with the platform list and carries no platform-managed credentials. Registry browsing is not currently available in the workspace management console; its **Add from registry** action is a disabled **Coming soon** placeholder.
