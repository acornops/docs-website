...
Runtime service-to-service calls use separate bearer tokens and are not part of the public browser API. External integration account-link, linked-user bot, and external webhook route connect/status endpoints are narrow exceptions: they accept an installed external integration client bearer token and derive the client from `EXTERNAL_INTEGRATION_CLIENTS_JSON`.
