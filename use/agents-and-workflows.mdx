...
- **Kubernetes Agent** is scoped to Kubernetes targets. It receives every read and write tool currently advertised by the selected target's built-in **AcornOps Kubernetes Tools** MCP server. Every write remains approval-gated.
...
remain unsupported. The automatic VM health check and starter VM Agent remain
...
| --- | --- | --- |
| Kubernetes health check | Automatic, active | One exact Kubernetes target. Reviews workload readiness, pod restarts, warning events, resource pressure, and relevant logs without making changes. |
| Virtual machine health check | Automatic, active | One exact Linux VM. Reviews host and filesystem pressure, processes, listeners, degraded systemd services, and allowlisted journal logs without making changes. |
