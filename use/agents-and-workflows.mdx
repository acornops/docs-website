...
operate on a Kubernetes target named in the request; this is prompt behavior,
not a persisted target binding.
...
Schedules store cadence and principal—not launch inputs or access grants. Every occurrence compiles the current active saved prompt and reauthorizes its explicit resource bindings and execution scope. An invalid definition, deletion, integration outage, membership change, or authorization failure auto-pauses the schedule with a bounded actionable reason; repair the cause and resume it manually.
...
approval, not as an execution binding.
