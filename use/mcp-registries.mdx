...
| --- | --- | --- | --- |
| One Agent | **Agents → Agent detail → Capabilities → MCP servers → Add MCP server → Browse registries** | Exact workspace and Agent ID, with optional authoritative target type/ID constraints | New runs using a pinned capability snapshot |
...
Every workflow step selects exactly one Agent. The workflow inherits that Agent's MCP, skill, and tool grants unless its capability policy narrows them; it cannot add a capability the Agent does not already grant. Target-owned tools remain target-scoped routes available through eligible Agent mappings and do not become workflow-owned credentials.

In `restrict` mode, an empty workflow restriction grants no capability. In `inherit` mode, the workflow receives the assigned Agent's eligible tools subject to the workflow's read/write mode.
...
See [MCP and tools](/use/tools-and-mcp) for direct endpoint rules, tool review, and runtime behavior, and [Configuration reference](/deploy/configuration-reference#mcp-egress-policy) for egress controls.
