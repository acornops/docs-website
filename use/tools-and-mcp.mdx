...
Semantic capability IDs do not authorize tools by themselves. For `target.diagnostics.read`, the control plane creates reviewed mappings from each eligible specialist Agent to each exact target after built-in tool synchronization. A Workflow does not select a target when it is authored or launched. Instead, the model supplies `target_id` when it calls a target MCP tool. The compiled authority is the intersection of the Agent target scope, the exact-target mapping, the acting user's permissions, and the live target catalog.
