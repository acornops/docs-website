...
5. For a write approval created by the integration's own troubleshooting run or Workflow execution, render the pending approval and require an explicit confirmation from the linked external user.
6. Submit the decision to `POST /api/v1/runs/{runId}/approvals/{approvalId}/decision` with the same linked integration credential, or direct the user to the management console. External credentials receive `403` for browser-created, other-link/client, standalone Agent, scheduled, or system-triggered approvals.
