...
## Linked read-only Workflow access
...
The linked identity is authorized only when the user has a live workspace role and saved grants for `read_workspace_data`, `create_sessions`, and `create_read_only_runs`, plus every permission required by the Workflow. Eligible definitions must be active, read-only, and have no approval requirements. AcornOps still applies Workflow V2 compilation, catalog readiness, typed prompt references, exact target bindings, and tool narrowing.
...
- `POST /api/v1/workflow-sessions/{sessionId}/messages` to dispatch a read-only execution in that owned session.

Each externally dispatched message should include a stable `clientRequestId`. Reuse it only when retrying the same provider message; use a new value for a new user message. This makes transport retries idempotent without making unrelated requests collide. External credentials cannot list arbitrary Workflow sessions, run write-capable definitions, satisfy approval-gated definitions, or use another user's session.
