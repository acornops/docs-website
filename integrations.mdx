...
AcornOps v1 does not expose a public PAT or broad bot service-account credential for the control-plane API. The deployment-owned `EXTERNAL_INTEGRATION_SERVICE_TOKEN` is only accepted by the external integration account-link endpoints.
...
Internal service tokens such as `ORCH_SERVICE_TOKEN`, `EXECUTION_ENGINE_DISPATCH_TOKEN`, `LLM_GATEWAY_ADMIN_TOKEN`, and `EXTERNAL_INTEGRATION_SERVICE_TOKEN` are platform credentials. They are not public integration credentials. The built-in MCP bridge uses the run-scoped JWT minted for the active execution.
...
- `POST /api/v1/auth/chat/integration/link` with `Authorization: Bearer <EXTERNAL_INTEGRATION_SERVICE_TOKEN>` and body `{ "externalUserId": "<external-user-id>" }` to create a short-lived browser link.
- `POST /api/v1/auth/chat/integration/resolve` with the same bearer token and body `{ "externalUserId": "<external-user-id>" }` to resolve whether the external identity is `linked` or `unlinked`.

AcornOps returns a management-console `linkUrl` under `/integrations/external-chat/link`. The user opens that URL within the token TTL, and the console shows the normal AcornOps login page when no browser session exists. After password or OIDC sign-in establishes a browser session, the console calls `POST /api/v1/auth/chat/integration/link/complete` with the link token to complete the durable account link.
